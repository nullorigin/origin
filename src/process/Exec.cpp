...
const string Run::RUN_STATES[17] = {
    ...
    "Killing",       "Killed"};
const string Run::CMD[16] = {"init",   "1", "start", "2", "pause",   "3",
                             "resume", "4", "stop",  "5", "restart", "6",
                             "exit",   "7", "kill",  "8"};
const u32 Run::CMD_LEN[16] = {4, 1, 5, 1, 5, 1, 6, 1, 4, 1, 7, 1, 4, 1, 4, 1};
const u32 Run::CMD_MAP[16] = {1, 1, 3,  3,  5,  5,  7,  7,
...
auto Run::getText(i32 name, const std::string &dlim) -> std::string {
  std::string txt[] = {
      (">-(" + con.getUser() + "@" + con.getHostname() + ")-$ [" + getInput() +
       "]" + dlim),
...
auto Run::processIn() -> Code {
  ...
  if ((Timer::now() >= Time_in_max) &&
      (rstatus != Code::Failure && rstatus != Code::Error)) {
...
auto Run::processIO() -> Code {
  const Code rstatus = getRunStatus();
  const bool success =
      ((processIn() == Code::Success) && (processOut() == Code::Success) &&
       (rstatus <= Code::Error) && (rstatus != Code::Failure));
