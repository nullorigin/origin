...
switch. Uses the constants above as specifiers for the equivalent index. */
        ...
        static inline auto getStateString(i32 rstate) -> string
        {
            return RUN_STATES[rstate];
        }
        auto getArg(const string& command) -> string;
        auto getCmd(const string& command) -> u32;
        ...
        inline auto getInput() const -> string { return Input; }
        ...
        // Reads input and writes output from various sources and to destinations, with actions similar to a shell.
        ...
        auto parse(const string& cmd) const -> string;
        auto getText(i32 name, const string& dlim = "\n") -> string;
    };
} // namespace origin
#endif