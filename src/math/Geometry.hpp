...
public:
  ...
  auto abs() const -> Vec2 { return {origin::abs(X), origin::abs(Y)}; }
  ...
  auto clamp(f64 min, f64 max) const -> Vec2 {
    return {origin::fmin(origin::fmax(X, min), max),
            origin::fmin(origin::fmax(Y, min), max)};
  }
  auto clamp(const Vec2 &min, const Vec2 &max) const -> Vec2 {
    return {origin::fmin(origin::fmax(X, min.X), max.X),
            origin::fmin(origin::fmax(Y, min.Y), max.Y)};
  }
  ...
  auto length() const -> f64 {
    return origin::abs(origin::sqrt(X * X + Y * Y));
  }
  ...
  auto rotate(f64 angle) const -> Vec2 {
    return {X * origin::cos(angle) - Y * origin::sin(angle),
            X * origin::sin(angle) + Y * origin::cos(angle)};
  }
...
public:
  ...
  Vec3(Vec3 &&) = default;
  Vec3(f64 *in) : X(in[0]), Y(in[1]), Z(in[2]) {}
  ~Vec3() = default;
  ...
  auto abs() const -> Vec3 {
    return {origin::abs(X), origin::abs(Y), origin::abs(Z)};
  }
  auto angle(const Vec3 &in) const -> f64 {
    return origin::acos(this->dot(in) / (this->length() * in.length()));
  }
  auto ceil() const -> Vec3 {
    return {origin::ceil(X), origin::ceil(Y), origin::ceil(Z)};
  }
  auto clamp(f64 min, f64 max) const -> Vec3 {
    return {origin::fmin(origin::fmax(X, min), max),
            origin::fmin(origin::fmax(Y, min), max),
            origin::fmin(origin::fmax(Z, min), max)};
  }
  auto clamp(const Vec3 &min, const Vec3 &max) const -> Vec3 {
    return {origin::fmin(origin::fmax(X, min.X), max.X),
            origin::fmin(origin::fmax(Y, min.Y), max.Y),
            origin::fmin(origin::fmax(Z, min.Z), max.Z)};
  }
  ...
  auto floor() const -> Vec3 {
    return Vec3(origin::floor(X), origin::floor(Y), origin::floor(Z));
  }
  auto length() const -> f64 {
    return origin::abs(origin::sqrt(X * X + Y * Y + Z * Z));
  }
  auto lengthSquared() const -> f64 {
    return origin::abs(X * X + Y * Y + Z * Z);
  }
  ...
  auto round() const -> Vec3 {
    return {origin::round(X), origin::round(Y), origin::round(Z)};
  }
  auto rotate(f64 angle) const -> Vec3 {
    return {X * origin::cos(angle) - Y * origin::sin(angle),
            X * origin::sin(angle) + Y * origin::cos(angle), Z};
  }
  auto trunc() const -> Vec3 {
    return {origin::trunc(X), origin::trunc(Y), origin::trunc(Z)};
  }
};
class Vec4 {
public:
  ...
  Vec4() = default;
  Vec4(Vec4 &&in) noexcept : X(in.X), Y(in.Y), Z(in.Z), W(in.W) {}
  Vec4(Vec4 const &in) = default;
  Vec4(f64 *in) : X(in[0]), Y(in[1]), Z(in[2]), W(in[3]) {}
  ~Vec4() = default;
  ...
  auto operator+(Vec4 const &v) const -> Vec4 {
  ...
  auto operator+(f64 in) const -> Vec4 {
  ...
  auto abs() const -> Vec4 {
    return {origin::abs(X), origin::abs(Y), origin::abs(Z), origin::abs(W)};
  }
  auto angle(const Vec4 &in) const -> f64 {
    return origin::acos(this->dot(in) / (this->length() * in.length()));
  }
  auto ceil() const -> Vec4 {
    return {origin::ceil(X), origin::ceil(Y), origin::ceil(Z), origin::ceil(W)};
  }
  auto clamp(f64 min, f64 max) const -> Vec4 {
    return Vec4(origin::fmin(origin::fmax(X, min), max),
                origin::fmin(origin::fmax(Y, min), max),
                origin::fmin(origin::fmax(Z, min), max),
                origin::fmin(origin::fmax(W, min), max));
  }
  auto clamp(const Vec4 &min, const Vec4 &max) const -> Vec4 {
    return Vec4(origin::fmin(origin::fmax(X, min.X), max.X),
                origin::fmin(origin::fmax(Y, min.Y), max.Y),
                origin::fmin(origin::fmax(Z, min.Z), max.Z),
                origin::fmin(origin::fmax(W, min.W), max.W));
  }
  ...
  auto floor() const -> Vec4 {
    return Vec4(origin::floor(X), origin::floor(Y), origin::floor(Z),
                origin::floor(W));
  }
  f64 length() const {
    return origin::abs(origin::sqrt(X * X + Y * Y + Z * Z + W * W));
  }
  f64 lengthSquared() const {
    return origin::abs(X * X + Y * Y + Z * Z + W * W);
  }
  Vec4 lerp(const Vec4 &in, f64 t) const { return (*this * (1 - t) + in * t); }
  Vec4 lerp(const Vec4 &v1, const Vec4 &v2, f64 t) const {
    return (*this * (1 - t) + v1 * t + v2 * (t - 1));
  }
  f64 max() const {
    return origin::fmax(X, origin::fmax(Y, origin::fmax(Z, W)));
  }
  Vec4 max(const Vec4 &v) const {
    return Vec4(origin::fmax(X, v.X), origin::fmax(Y, v.Y),
                origin::fmax(Z, v.Z), origin::fmax(W, v.W));
  }
  f64 min() const {
    return origin::fmin(X, origin::fmin(Y, origin::fmin(Z, W)));
  }
  Vec4 min(const Vec4 &v) const {
    return Vec4(origin::fmin(X, v.X), origin::fmin(Y, v.Y),
                origin::fmin(Z, v.Z), origin::fmin(W, v.W));
  }
  ...
  Vec4 rotate(f64 angle) const {
    return {X * origin::cos(angle) - Y * origin::sin(angle),
            X * origin::sin(angle) + Y * origin::cos(angle), Z, W};
  }
  Vec4 trunc() const {
    return {origin::trunc(X), origin::trunc(Y), origin::trunc(Z),
            origin::trunc(W)};
...
public:
  Vec2 Min;
  Vec2 Max;
  Rect(Vec2 v1, Vec2 v2) : Min(v1), Max(v2) {}
  explicit Rect(Vec4 v) {
    Min = Vec2(v.X, v.Y);
    Max = Vec2(v.Z, v.W);
  }
  ...
  auto area() const -> f64 {
    return origin::abs((Max.X - Min.X) * (Max.Y - Min.Y));
  }
  ...
  Rect rotate(f64 a) const { return Rect(Min.rotate(a), Max.rotate(a)); }
...
class Matrix {
public:
  f64 X0{}, X1{}, X2{}, X3{}, Y0{}, Y1{}, Y2{}, Y3{}, Z0{}, Z1{}, Z2{}, Z3{},
      W0{}, W1{}, W2{}, W3{};
  Matrix() {
    X0 = X1 = X2 = X3 = Y0 = Y1 = Y2 = Y3 = Z0 = Z1 = Z2 = Z3 = W0 = W1 = W2 =
        W3 = 0.0;
  }

  Matrix(f64 x0, f64 x1, f64 x2, f64 x3, f64 y0, f64 y1, f64 y2, f64 y3, f64 z0,
         f64 z1, f64 z2, f64 z3, f64 w0, f64 w1, f64 w2, f64 w3)
      : X0(x0), X1(x1), X2(x2), X3(x3), Y0(y0), Y1(y1), Y2(y2), Y3(y3), Z0(z0),
        Z1(z1), Z2(z2), Z3(z3), W0(w0), W1(w1), W2(w2), W3(w3) {}

  Matrix(Matrix &&m) noexcept
      : X0(m.X0), X1(m.X1), X2(m.X2), X3(m.X3), Y0(m.Y0), Y1(m.Y1), Y2(m.Y2),
        Y3(m.Y3), Z0(m.Z0), Z1(m.Z1), Z2(m.Z2), Z3(m.Z3), W0(m.W0), W1(m.W1),
        W2(m.W2), W3(m.W3) {}
  Matrix(const Matrix &m)
      : Matrix(m.X0, m.X1, m.X2, m.X3, m.Y0, m.Y1, m.Y2, m.Y3, m.Z0, m.Z1, m.Z2,
               m.Z3, m.W0, m.W1, m.W2, m.W3) {}
  ~Matrix() = default;
  ...
  auto operator+(const Matrix &m) const -> Matrix {
    return {X0 + m.X0, X1 + m.X1, X2 + m.X2, X3 + m.X3, Y0 + m.Y0, Y1 + m.Y1,
            Y2 + m.Y2, Y3 + m.Y3, Z0 + m.Z0, Z1 + m.Z1, Z2 + m.Z2, Z3 + m.Z3,
  ...
  auto rotate(Vec3 &v) const -> Vec3 {
    return {X0 * v.X + Y0 * v.Y + Z0 * v.Z, X1 * v.X + Y1 * v.Y + Z1 * v.Z,
            X2 * v.X + Y2 * v.Y + Z2 * v.Z};
  }
  auto translate(Vec3 &v) const -> Vec3 {
    return {X3 * v.X + Y3 * v.Y + Z3 * v.Z, X3 * v.X + Y3 * v.Y + Z3 * v.Z,
...
public:
  ...
  Quaternion(f64 x0, f64 x1, f64 x2, f64 x3, f64 y0, f64 y1, f64 y2, f64 y3,
             f64 z0, f64 z1, f64 z2, f64 z3, f64 w0, f64 w1, f64 w2, f64 w3)
      : X0(x0), X1(x1), X2(x2), X3(x3), Y0(y0), Y1(y1), Y2(y2), Y3(y3), Z0(z0),
        Z1(z1), Z2(z2), Z3(z3), W0(w0), W1(w1), W2(w2), W3(w3) {}
  Quaternion(Quaternion const &) = default;

  explicit Quaternion(const f64 *in) noexcept
      : X0{in[0]}, X1{in[1]}, X2{in[2]}, X3{in[3]}, Y0{in[4]}, Y1{in[5]},
        Y2{in[6]}, Y3{in[7]}, Z0{in[8]}, Z1{in[9]}, Z2{in[10]}, Z3{in[11]},
        W0{in[12]}, W1{in[13]}, W2{in[14]}, W3{in[15]} {}
  Quaternion(const f64 x[4], const f64 y[4], const f64 z[4],
             const f64 w[4] = nullptr) noexcept
      : X0{x[0]}, X1{x[1]}, X2{x[2]}, X3{x[3]}, Y0{y[0]}, Y1{y[1]}, Y2{y[2]},
        Y3{y[3]}, Z0{z[0]}, Z1{z[1]}, Z2{z[2]}, Z3{z[3]},
        W0{(w != nullptr) ? w[0] : 0.0}, W1{(w != nullptr) ? w[1] : 0.0},
        W2{(w != nullptr) ? w[2] : 0.0}, W3{(w != nullptr) ? w[3] : 1.0} {}
  ...
  static auto transform(const Quaternion &q, const Vec3 &v) {
    ...
    return Quaternion(x0, x1, x2, x3, y0, y1, y2, y3, z0, z1, z2, z3, 0.0, 0.0,
                      0.0, 1.0);
  }
  ...
  static auto rotate(const Quaternion &q, Vec3 &axis, Face f) -> Quaternion {
    f64 const a =
        acos(q.X0) +
        PI * 2 *
            ((axis.X * axis.X + axis.Y * axis.Y + axis.Z * axis.Z) * 2.0 - 1.0);
    Vec3 dv{};
    if (f == Face::Right) {
      dv.X = origin::sin(a);
    }
  ...
  [[nodiscard]] auto dot(const Quaternion &in, const Vec3 &t) const -> f64 {
    return X0 * in.X0 + Y0 * in.Y0 + Z0 * in.Z0 + W0 * in.W0 + t.X * in.X0 +
           t.Y * in.Y0 + t.Z * in.Z0 + t.X * in.X0 + t.Y * in.Y0 + t.Z * in.Z0;
  }
  [[nodiscard]] auto lengthSquared() const -> f64 {
    return dot(*this, Vec3(X0, Y0, Z0));
  }
  ...
  auto toString() const noexcept -> std::string {
    std::string out = "\n";
    auto out0 = "Quaternion(" + std::to_string(X0) + ", " + std::to_string(Y0) +
                ", " + std::to_string(Z0) + ", " + std::to_string(W0) + ",\n";
    auto out1 = std::to_string(X1) + ", " + std::to_string(Y1) + ", " +
                std::to_string(Z1) + ", " + std::to_string(W1) + ",\n";
    auto out2 = std::to_string(X2) + ", " + std::to_string(Y2) + ", " +
                std::to_string(Z2) + ", " + std::to_string(W2) + ",\n";
    auto out3 = std::to_string(X3) + ", " + std::to_string(Y3) + ", " +
                std::to_string(Z3) + ", " + std::to_string(W3) + ");\n";
    out += out0 + std::string(11, ' ') + out1 + std::string(11, ' ') + out2 +
           std::string(11, ' ') + out3;
    return out;
  }
  ...
  operator f64 *() { return cast<f64 *>(*this); }
  ...
  auto operator!=(const Quaternion &q) const -> bool { return !(*this == q); }
};
} // namespace origin
#endif // GEOMETRY_HPP
